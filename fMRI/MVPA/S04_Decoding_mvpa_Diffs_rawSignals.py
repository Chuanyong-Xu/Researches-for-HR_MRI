...
# from imblearn.over_sampling import RandomOverSampler # ******for upsampling only******
...
common_settings.model_name = 'mvpa_Diffs_KFolds' 
common_settings.label_map = {1: 1, 2: 2, 3:3} ######### change*
common_settings.concepts_map = {1:'Hard', 2:'Mid', 3:'Easy'} ######### change*

#==============================================================================
# train and test
subs=[1101, 1104, 1105, 1108, 1110, 1114, 1116, 1117, 1120, 1121]
#, 1132, 1141, 1142, 1144, 1146, 1147, 1148, 1150, 1155, 1157, 1160, 1161, 1162, 1164, 1165]
for sub in subs:
    if __name__ == "__main__":
        [os.remove(item) for item in glob('../*/core*')]
        radius = common_settings.radius
        cond_cat  = 'trials'
        condition = 'type_dev_error_2' #(dif) according to experiment conditions to change ################# change*
        ...
        results_dir = '../results/KFolds_MNI_space_Diffs_raws10_mu' ################## change*
#   --------------- 
...
# for MVPA train and test, searchlight
    ...
    weightsM = weights[:,1]
    weightsH = weights[:,0]
    weightsE = masker.inverse_transform(weightsE.reshape(1,-1))
    weightsM = masker.inverse_transform(weightsM.reshape(1,-1))
    weightsH = masker.inverse_transform(weightsH.reshape(1,-1))
    weights_brain_name = os.path.join(results_dir,
                                           f'{sub}_{condition}_{radius}mm_weightsE_brain.nii.gz')
    nib.save(weightsE, weights_brain_name)
    weights_brain_name = os.path.join(results_dir,
                                           f'{sub}_{condition}_{radius}mm_weightsM_brain.nii.gz')
    nib.save(weightsM, weights_brain_name)      
    weights_brain_name = os.path.join(results_dir,
                                           f'{sub}_{condition}_{radius}mm_weightsH_brain.nii.gz')
    nib.save(weightsH, weights_brain_name)      
