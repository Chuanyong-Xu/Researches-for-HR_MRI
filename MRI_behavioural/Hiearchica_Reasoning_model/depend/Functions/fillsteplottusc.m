...
for c=1:size(dat,2);
    ...
    hold on; 
    h(c)=plot(xrange,m,'color',[0.2784    0.4471    0.4510],'LineWidth',linewid,'linestyle',linez{c});
    
    d = squeeze(dat(:,c,:));
   
%    md = nanmean(d);
    fill_around_line(m,ste(d),[0.2784    0.4471    0.4510],xrange);
...
elseif ndims(dat)==2;
   m=squeeze(nanmean(dat));
    ...
    hold on; 
    h=plot(m,'color',[0.2784    0.4471    0.4510],'LineWidth',linewid,'linestyle',linez{1});
       
%    md = nanmean(d);
    % % % % % % % % fill_around_line(m,ste(dat),[0.2784    0.4471    0.4510]);%*****by xu20230926
    ...
    fill(x_fill,y_fill,[0.2784    0.4471    0.4510],'FaceAlpha',0.2,'EdgeColor','none')
    %*****by xu20230926
