...
for c=1:size(dat,2);
    ...
    hold on; 
    h(c)=plot(xrange,m,'color',[1 0.84 0],'LineWidth',linewid,'linestyle',linez{c});
    
    d = squeeze(dat(:,c,:));
   
%    md = nanmean(d);
    fill_around_line(m,ste(d),[1 0.84 0],xrange);
...
elseif ndims(dat)==2;
   m=squeeze(nanmean(dat));
    ...
    hold on; 
    h = plot(m,'color',[1 0.84 0],'LineWidth',linewid,'linestyle',linez{1});
...
% % % %     fill_around_line(m,ste(dat),[0.4660 0.6740 0.1880]);%*****by xu20230926
    ...
    fill(x_fill,y_fill,[1 0.84 0],'FaceAlpha',0.2,'EdgeColor','none')
    %*****by xu20230926
