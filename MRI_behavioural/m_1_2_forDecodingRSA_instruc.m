...
sub_data=sub_data(sub_data(:,3)==0,:); % only exp=0(instructed)


Nback_act =[];
fstIdx = find(sub_data(:,expIdxIdx) ==0);

    for iTrial = fstIdx(1):fstIdx(1)+length(fstIdx)-1
        if iTrial == fstIdx(1) %fstIdx(1)=151, the first trial
            Nback_act(iTrial,1) = double(~sub_data(iTrial,feedbackIdx));%the Nback of 151's trial = the reversal feedback of 151's；(~1)=0
        else %otherwise(152-300's trial) is equal to：
            Nback_act(iTrial,1) = (double(~sub_data(iTrial,feedbackIdx)) * (Nback_act(iTrial-1) + 1))*  (~sub_data(iTrial-1,ruleSwitchIdx))+  (~sub_data(iTrial,feedbackIdx)) * sub_data(iTrial-1,ruleSwitchIdx);%changed by wenshan 20220518
        end
    end
Nback_act(Nback_act>=2)=2;
sub_data(:,26) = Nback_act;
...
for ii=1:length(sub_data)
    ...
    elseif 101<=sub_data(ii,4) && sub_data(ii,4)<=150
        if subj==20
            time_run=1148+1080; chunks(ii,1)=3;
        else
