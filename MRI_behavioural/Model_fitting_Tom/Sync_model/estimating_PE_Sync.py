...
os.chdir('/Users/vincentxu/Desktop/Model/IOM-CBM-5points-MRI/MRI_behavioural-29subs/Beha_ModCom/Sync_data')
...
# pars = [Cumulation, Learning low, Learning high, Threshold, Temperature]
# Threshold is fixed at 0.5, so it is not counted as a free parameter.
k = len(pars[0]) - 1
AIC = -2 * np.asarray(Lik) + 2 * k
print("Mean AIC")
print(np.mean(AIC))
print("std AIC")
print(np.std(AIC))
...
for p in pplist:
    ...
    ntrials.append(data.shape[0])
ntrials = np.asarray(ntrials)
BIC = np.log(ntrials) * k - 2 * np.asarray(Lik)
