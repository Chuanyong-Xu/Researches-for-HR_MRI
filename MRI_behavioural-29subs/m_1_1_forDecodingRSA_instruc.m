...
exclud1=[]%[2 6 10 32 33 35];%[7 14 16 19];%%%%%%[20 21 25 27 33 35];
dir1(exclud1,:)=[];

dir0 = dir([matdir,filesep,'*instruct_exp*.mat']);
exclud2=[]%[2 6 10 32 33 35];%[7 14 16 19];%%%%%%[10 20];% excluding the subjects with low accuracy or bad quality
dir0(exclud2,:)=[];

load data_28sub_raw.mat;%***********
...
load model_res_switch_fmri_m_4_28sub.mat;
idIdx         = 1;
expIdxIdx     = 3;
trialnumIdx   = 4;
rulelistIdx   = 5;
...
sub_data(:,26)=Input(subj).Nback; %error type
sub_data=sub_data(sub_data(:,3)==0,:); % exp=1 (inference) exp=0 (instructed)

% after only includ the instructed task data, constructing the N-back index
conditions_1Back_index = find((sub_data(1:end-2,expIdxIdx) == 0) .* (sub_data(1:end-2,feedbackIdx)==1) .* (sub_data(2:end-1,feedbackIdx)==0)) +1; % RW, ER  => {1B-Er}
conditions_2Back_index = find((sub_data(1:end-3,expIdxIdx) == 0) .* (sub_data(1:end-3,feedbackIdx)==1) .* (sub_data(2:end-2,feedbackIdx)==0) .* (sub_data(3:end-1,feedbackIdx)==0) .* (sub_data(2:end-2,ruleRespIdx) == sub_data(3:end-1,ruleRespIdx))) +2; % RW, ER, ER  => {2B-Er}
sub_data(conditions_1Back_index,26)=1; %error type
sub_data(conditions_2Back_index,26)=2;%error type
sub_data((sub_data(:,26)==0 & sub_data(:,3)==0 & sub_data(:,14)==0),26)=2;
% for ruleRespIdx
% ---------------------sub_data_ruleresp---------------------
sub_data(:,24)=ceil((sub_data(:,17)+0.85)/0.85); %onsetFB--feedback duration
...
writetable(T,['decoding_rsa_behaviors_instructed/' 'data_raw_decodingRSA_ruleresponse_' num2str(id(subj)) '.csv']);
clear chunks context time_coords time_indices trials rule_respons T

%---------------------for bias_levelIdx---------------------
sub_data(:,24)=ceil((sub_data(:,18)+0.85)/0.85); %onsetFB--feedback duration
...
Dev_lev(Dev_lev==1 | Dev_lev==5)=1; Dev_lev(Dev_lev==2 | Dev_lev==4)=2; %%%%%%%%
pcptResp=sub_data(:,pcptRespIdx); 
T=table(chunks,context,targets,time_coords,time_indices,trials,Dev_lev,pcptResp);
writetable(T,['decoding_rsa_behaviors_instructed/' 'data_raw_decodingRSA_Dev_lev_' num2str(id(subj)) '.csv']);
clear chunks context time_coords time_indices trials rule_respons T

%% ---------------------for confidence---------------------
% ***======================================================================
sub_data(:,24)=ceil((sub_data(:,16)+0.85)/0.85); %onsetFB--feedback duration
...
type_dev_error(type_dev_error(:,1)==0 & type_dev_error(:,2)==1,4)=3; type_dev_error(type_dev_error(:,1)==0 & type_dev_error(:,2)==2,4)=2;
type_dev_error(type_dev_error(:,1)==0 & type_dev_error(:,2)==3,4)=1; type_dev_error(type_dev_error(:,1)==1 & type_dev_error(:,2)==1,4)=6;
type_dev_error(type_dev_error(:,1)==1 & type_dev_error(:,2)==2,4)=5; type_dev_error(type_dev_error(:,1)==1 & type_dev_error(:,2)==3,4)=4;
type_dev_error(type_dev_error(:,1)>=2 & type_dev_error(:,2)==1,4)=9; type_dev_error(type_dev_error(:,1)>=2 & type_dev_error(:,2)==2,4)=8;
type_dev_error(type_dev_error(:,1)>=2 & type_dev_error(:,2)==3,4)=7;

confidence=sub_data(:,confidenceIdx); confidence(confidence == Inf)=1e+2;
...
writetable(T,['decoding_rsa_behaviors_instructed/' 'data_raw_decodingRSA_confidence_' num2str(id(subj)) '.csv']);
...
writetable(T,['decoding_rsa_behaviors_instructed/' 'data_raw_decodingRSA_ruleSwitch_' num2str(id(subj)) '.csv']);
clear chunks context time_coords time_indices trials rule_respons T
