...
% figure layout
    figure;
    set(gcf,'units','centimeters','position',[2 5 50 20]);
...
%Inferred: subjective rules
    ...
    ylabel('Pr(Anti | Rule, Angle)','fontsize',14); 
    xlabel('Probability of choice of Anti','fontsize',14);  
    ...
    xlabel('Probability of choice of Anti');  
    hold on
...
for num =1:5 %******
    ...
    xlabel('Trial after rule switch')
...
for ki = 1:3
    ...
    ylabel('Pr(isSwitch)','fontsize',12)
    set(gca,'FontSize',14) 
    ...
    set(gca,'tickdir','out')
    set(gca,'box','off', 'TitleHorizontalAlignment','center')
    legend([h3,h2,h1],{'2B-Er','1B-Er','Rw'},'Position',[0.88 0.8 0.1 0.1],'Orientation','vertical'); legend boxoff;%'location', 'SouthEast' 
    set(gca,'FontSize',14) 
...
%1st level
    ...
    % % % AIC_2nd_m(1) = mean(AIC_2nd)
    % % % BIC_2nd_m(1) = mean(BIC_2nd)
...
addpath(genpath('/Users/vincentxu/Desktop/Model/IOM-CBM-5points-MRI/RNN-29subs/matlab-special-heatmap'));
    figure
    set(gcf,'unit','pixels','position',[500 50 400 250]);
    set(gca,'Fontsize',14);
    hbar            = bar(modcomp_m);
    ...
    legend('Log-lokelihood','AIC','BIC','Location','northwest')
    title('Model comparasion','fontsize',18,'FontWeight','normal')
    box off;grid off
