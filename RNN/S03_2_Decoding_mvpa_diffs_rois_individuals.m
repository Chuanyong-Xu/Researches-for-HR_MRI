...
figure; set(gcf,'unit','pixels','position',[200 50 1600 1600]);
    ...
    for subs = 1:length(nii_dir)
        ...
        nii_dir_sub_data{subs}(:,1) = mean(nii_dir_sub_data9{subs}(:,[1 4 7]),2);
        nii_dir_sub_data{subs}(:,2) = mean(nii_dir_sub_data9{subs}(:,[2 5 8]),2);
        nii_dir_sub_data{subs}(:,3) = mean(nii_dir_sub_data9{subs}(:,[3 6 9]),2);
        ...
        percent_max(subs,1) = sum(nii_dir_sub_data_max{subs}==1)./length(nii_dir_sub_data_max{subs});
        percent_max(subs,2) = sum(nii_dir_sub_data_max{subs}==2)./length(nii_dir_sub_data_max{subs});
        percent_max(subs,3) = sum(nii_dir_sub_data_max{subs}==3)./length(nii_dir_sub_data_max{subs});
...
%         nii_dir_sub_data{subs} = (nii_dir_sub_data{subs})./max(nii_dir_sub_data{subs},[],2); % max value scale
        ...
        colorlist = flip(slanCM(77));%ner: 2 dif :77
        colors=[colorlist(20,:); colorlist(50,:); colorlist(90,:)];
            mu0 = mean(nii_dir_sub_data{subs}(nii_dir_sub_data_max{subs}==1,:),1);
            plot([1:3],mu0,'-o','LineWidth',2,'Color',colors(1,:),'MarkerFaceColor',colors(1,:),'MarkerEdgeColor',colors(1,:));    
            mu1 = mean(nii_dir_sub_data{subs}(nii_dir_sub_data_max{subs}==2,:),1);
            plot([1:3],mu1,'-o','LineWidth',2,'Color',colors(2,:),'MarkerFaceColor',colors(2,:),'MarkerEdgeColor',colors(2,:));    
            mu2 = mean(nii_dir_sub_data{subs}(nii_dir_sub_data_max{subs}==3,:),1);
            plot([1:3],mu2,'-o','LineWidth',2,'Color',colors(3,:),'MarkerFaceColor',colors(3,:),'MarkerEdgeColor',colors(3,:));

        legend({'Hard','Mid','Easy'}, 'Location','NorthEast'); 
        xlabel('Difficulties'); ylabel('Weights');
        hold off
...
print(['figures/' 'Diffs_weights_density_rois_subs_' num2str(subs)],'-dsvg','-painters');
% close all
    ...
    set(gcf,'unit','pixels','position',[200 50 400 200]);
        ...
        legend({'Hard','Mid','Easy'}, 'Location','NorthEast'); 
        xlabel('Difficulties'); ylabel('Normalized Weights');
        xticks([0 1 2]); xticklabels({'D1','D2','D3'});
        hold off
print(['figures/' 'Diffs_weights_density_rois_subsMean'],'-dsvg','-painters');
close all 
    %% plot the gaussan
    figure; hold on
    set(gcf,'unit','pixels','position',[200 50 400 200]);
...
%     semw        = sigma./sqrt(length(nii_dir_sub_data(nii_dir_sub_data_max==1,:)));
    ...
    optsG       = fitoptions('Method', 'NonlinearLeastSquares', 'Start',...
        [max(means), errs(means==max(means)),1], 'Lower',[0,1,0],'Upper',[1,1,Inf],'Display','off');%,'Weights',1./(semw.^2)
    ...
    plot(errs, means,'o','MarkerSize',6,'MarkerFaceColor',colors(1,:),'MarkerEdgeColor',colors(1,:))
            R2_dif(1) = gofg.rsquare;
            fprintf('Gaussian D1 R² = %.4f\n', R2_dif(1));
%  
    ...
    gauss       = fittype('A*exp(-((x-mu) .^2)/(2*sigma^2))','coeff',{'A','mu','sigma'});
    optsG       = fitoptions('Method', 'NonlinearLeastSquares', 'Start',...
        [max(means), errs(means==max(means)),1], 'Lower',[0,2,0],'Upper',[1,2,Inf],'Display','off');
    [cfung, gofg] = fit(errs',means',gauss,optsG); %*****************
    ygauss      = cfung.A * exp(-((xFine-cfung.mu) .^2)/(2*cfung.sigma^2));
    ...
    plot(errs, means,'o','MarkerSize',6,'MarkerFaceColor',colors(2,:),'MarkerEdgeColor',colors(2,:))
            R2_dif(2) = gofg.rsquare;
            fprintf('Gaussian D2 R² = %.4f\n', R2_dif(2));
%    
    ...
    gauss       = fittype('A*exp(-((x-mu) .^2)/(2*sigma^2))','coeff',{'A','mu','sigma'});
    optsG       = fitoptions('Method', 'NonlinearLeastSquares', 'Start',...
        [max(means), errs(means==max(means)),1], 'Lower',[0,3,0],'Upper',[1,3,Inf],'Display','off');
    [cfung, gofg] = fit(errs',means',gauss,optsG); %*****************
    ygauss      = cfung.A * exp(-((xFine-cfung.mu) .^2)/(2*cfung.sigma^2));
    ...
    plot(errs, means,'o','MarkerSize',6,'MarkerFaceColor',colors(3,:),'MarkerEdgeColor',colors(3,:))
            R2_dif(3) = gofg.rsquare;
            fprintf('Gaussian D3 R² = %.4f\n', R2_dif(3));

    legend({'Hard','Hard','Mid','Mid','Easy','Easy'}, 'Location','NorthWest'); 
    xlabel('Difficulties'); ylabel('Normalized Activations')
    xticks([1 2 3]); xticklabels({'D1','D2','D3'});
    hold off
print(['figures/' 'Diffs_weights_density_rois_subsMean_gau'],'-dsvg','-painters');
% close all
