...
for e = 1:numel(epoch_files)
    ...
    for subs = 1:29
        ...
        neur_RDM_e(subs,:) = (neur_cond_RDM');
        neur_cond_RDM = pdist((mask_data'), 'correlation');
        neur_RDM_c(subs,:) = (neur_cond_RDM');
    end
    ...
    for b = 1:boot_n
        ...
        beha_d_fixed = beha_d_all(:,fixed_it);
        VarNames = {'beha_d','neur_cond_RDM','subj_c'};
        tbl_fixed = array2table([beha_d_fixed,reshape(neur_RDM_e.', [], 1),subj_c],'VariableNames',VarNames);
        formula = 'neur_cond_RDM ~ 1 + beha_d + (1 + beha_d | subj_c)';
        fit_fixed = fitlme(tbl_fixed,formula);
        beta_fixed_epoch_e(e) = fit_fixed.Coefficients.Estimate(2);
        % correlation
        beha_d_fixed = beha_d_all(:,fixed_it);
        VarNames = {'beha_d','neur_cond_RDM','subj_c'};
        tbl_fixed = array2table([beha_d_fixed,reshape(neur_RDM_c.', [], 1),subj_c],'VariableNames',VarNames);
        formula = 'neur_cond_RDM ~ 1 + beha_d + (1 + beha_d | subj_c)';
        fit_fixed = fitlme(tbl_fixed,formula);
        beta_fixed_epoch_c(e) = fit_fixed.Coefficients.Estimate(2);
...
hE(1)=plot(epoch_id, beta_fixed_epoch, 'o-', 'LineWidth',2.2, ...
    'MarkerFaceColor','w', 'MarkerSize',5.5, 'Color',colBlue);
hE(2)=plot(epoch_id, beta_fixed_epoch_e', 's-', 'LineWidth',2.2, ...
    'MarkerFaceColor','w', 'MarkerSize',5.5, 'Color',colTeal);
hE(3)=plot(epoch_id, beta_fixed_epoch_c', '^-', 'LineWidth',2.2, ...
    'MarkerFaceColor','w', 'MarkerSize',5.5, 'Color',colPurple);
yline(0,'--','LineWidth',1.1,'Color',colZero);
xlabel('Epoch');
ylabel('\beta for model: [90^\circ, 1]');
legend(hE, {'Mahalanobis','Euclidean','Correlation'}, ...
