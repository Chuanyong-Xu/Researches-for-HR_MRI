...
ylabel('Pr(Switch)','fontsize',14); xlabel('Difficulties','fontsize',14)
...
colorlist = flip(slanCM(21));
%plot RDM----------------------------------------------------------------
for subs=1:29
    conds_9_subs = reshape(conds_9(subs,:,:),size(conds_9,2),1*size(conds_9,3));
    conds_9_subs_all(subs,:,:) = zscore(conds_9_subs,0,2);
end

data_RDM = reshape(mean(conds_9_subs_all,1),size(conds_9_subs_all,2),1*size(conds_9_subs_all,3));
% data_RDM = conds_9_subs_all(:,:,27);
